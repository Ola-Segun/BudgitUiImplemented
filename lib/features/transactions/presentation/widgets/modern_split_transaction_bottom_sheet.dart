...
/// Modern split transaction bottom sheet with transformed UI
class ModernSplitTransactionBottomSheet extends ConsumerWidget {
  const ModernSplitTransactionBottomSheet({
    super.key,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return _ModernSplitTransactionContent(
      onSubmit: onSubmit,
  ...
  static Future<T?> show<T>({
    ...
    try {
      return await showModernBottomSheet<T>(
        context: context,
        builder: (context) => ModernSplitTransactionBottomSheet(
          onSubmit: onSubmit,
          initialType: initialType,
          initialAmount: initialAmount,
        ),
...
class _ModernSplitTransactionContent extends ConsumerStatefulWidget {
  const _ModernSplitTransactionContent({
    required this.onSubmit,
    this.initialType,
    this.initialAmount,
  });
  ...
  @override
  ConsumerState<_ModernSplitTransactionContent> createState() =>
      _ModernSplitTransactionState();
}

class _ModernSplitTransactionState extends ConsumerState<_ModernSplitTransactionContent> {
  ...
  void initState() {
    ...
    if (widget.initialAmount != null && widget.initialAmount! > 0) {
      _totalAmountController.text = widget.initialAmount!.toStringAsFixed(0);
    }
